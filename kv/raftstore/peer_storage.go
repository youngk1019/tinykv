...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	preLast, err := ps.LastIndex()
	if err != nil {
		return err
	}
	for i := last + 1; i <= preLast; i++ {
		raftWB.DeleteMeta(meta.RaftLogKey(regionId, i))
	}
...
func (ps *PeerStorage) ApplySnapshot(snapshot *eraftpb.Snapshot, kvWB *engine_util.WriteBatch, raftWB *engine_util.WriteBatch) (*ApplySnapResult, error) {
	...
	if ps.isInitialized() {
		...
		if err != nil {
			log.Errorf("peer storage clear meta error: %s", err.Error())
			panic(err)
		}
	...
	if err != nil {
		log.Errorf("kv wb set meta error: %s", err.Error())
		panic(err)
	}
...
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	if !raft.IsEmptySnap(&ready.Snapshot) {
		...
		if err != nil {
			log.Errorf("peer storage save ready state, apply snapshot error: %s", err.Error())
			panic(err)
		}
