...
func (state *SoftState) equal(ob *SoftState) bool {
	return state.RaftState == ob.RaftState && state.Lead == ob.Lead
}
...
func (rn *RawNode) Ready() Ready {
	...
	if isHardStateEqual(rn.preHardState, hardState) {
...
func (rn *RawNode) HasReady() bool {
	// Your Code Here (2A).
	if !rn.Raft.softState().equal(rn.preSoftState) {
		return true
	}
	if hardState := rn.Raft.hardState(); !isHardStateEqual(rn.preHardState, hardState) && !IsEmptyHardState(hardState) {
		return true
	}
